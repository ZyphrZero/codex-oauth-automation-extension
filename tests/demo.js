...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/564876/ChatGPT%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/564876/ChatGPT%20Tools.meta.js
// ==/UserScript==
...
(function () {
    ...
    const _0x_MM_DEFAULT_API_BASE = 'https://sall.cc';
    const _0x_AUTH_LOGIN_URL = _0xb('aHR0cHM6Ly9jaGF0Z3B0LmNvbS9hdXRoL2xvZ2lu');
    const _0x_AUTH_SESSION_URL = 'https://chatgpt.com/api/auth/session';
    const _0x_MM_KEYS = {
        API_BASE_URL: 'moemail_api_base_url',
...
`);
    ...
    /* ═══════════════ MoeMail API ═══════════════ */
    function _0x_mmNormBaseUrl(value) {
        const input = (value || '').trim() || _0x_MM_DEFAULT_API_BASE;
        let url;
        try {
            url = new URL(input);
        } catch (e) {
            throw new Error('MoeMail API Base URL 格式不正确');
        }
    ...
    function _0x_mmGetBaseUrl() {
        return _0x_mmNormBaseUrl(GM_getValue(_0x_MM_KEYS.API_BASE_URL, _0x_MM_DEFAULT_API_BASE));
    }

    function _0x_mmGetKey() {
        return (GM_getValue(_0x_MM_KEYS.API_KEY, '') || '').trim();
    }

    function _0x_mmParseDomains(config) {
        if (Array.isArray(config?.domains)) {
            return config.domains
                .map(item => typeof item === 'string' ? item : item?.domain)
                .filter(Boolean);
        }
        const raw = config?.emailDomains || config?.email_domains || '';
        return String(raw)
            .split(',')
            .map(s => s.trim())
            .filter(Boolean);
    ...
    function _0x_mmReq(endpoint, options = {}, overrides = {}) {
        const apiKey = (overrides.apiKey || _0x_mmGetKey()).trim();
        let apiBaseUrl;
        try {
            apiBaseUrl = overrides.apiBaseUrl ? _0x_mmNormBaseUrl(overrides.apiBaseUrl) : _0x_mmGetBaseUrl();
        } catch (e) {
            return Promise.reject(e);
        }
        if (!apiKey) return Promise.reject(new Error('请先配置 MoeMail API Key'));
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (res) {
                    try {
                        ...
                        else reject(new Error(data.error || data.message || 'HTTP ' + res.status));
    ...
    async function _0x_mmDomains(apiBaseUrl, apiKey) {
        const config = await _0x_mmReq('/api/config', {}, { apiBaseUrl, apiKey });
        const domains = _0x_mmParseDomains(config);
        if (domains.length === 0) throw new Error('未获取到 MoeMail 可用域名');
        return domains;
    }

    async function _0x_mmListEmails() {
        let all = [];
        let cursor = null;
        do {
            const url = '/api/emails' + (cursor ? '?cursor=' + cursor : '');
            const res = await _0x_mmReq(url);
            if (Array.isArray(res?.emails)) all = all.concat(res.emails);
            cursor = res?.nextCursor || null;
        } while (cursor);
        return all;
    }

    async function _0x_mmFindEmailByAddress(address) {
        const all = await _0x_mmListEmails();
        return all.find(item => item?.email === address || item?.address === address) || null;
    }

    async function _0x_mmCreateEmail(domain = null, prefix = null) {
        const domains = domain ? [domain] : await _0x_mmDomains();
        const body = {
            expiryTime: 0,
            domain: domains[0],
            name: prefix || _0x_randPfx(10)
        };
        const res = await _0x_mmReq('/api/emails/generate', { method: 'POST', body });
        if (res?.id && res?.email) return { address: res.email, id: res.id };
        throw new Error('MoeMail 创建邮箱失败');
    }

    async function _0x_mmExtractCode(address, emailId) {
        let targetId = emailId;
        if (!targetId) {
            const emailInfo = await _0x_mmFindEmailByAddress(address);
            targetId = emailInfo?.id || null;
        }
        if (!targetId) return null;
        const list = await _0x_mmReq('/api/emails/' + targetId);
        if (!Array.isArray(list?.messages) || list.messages.length === 0) return null;
        const msg = await _0x_mmReq('/api/emails/' + targetId + '/' + list.messages[0].id);
        const content = (msg?.message?.content || '') + ' ' + (msg?.message?.html || '') + ' ' + (msg?.message?.subject || '');
    ...
    async function _0x_mmFetchMails(address, emailId) {
        let targetId = emailId;
        if (!targetId) {
            const emailInfo = await _0x_mmFindEmailByAddress(address);
            targetId = emailInfo?.id || null;
        ...
        for (const item of list.messages) {
            try {
                const detail = await _0x_mmReq('/api/emails/' + targetId + '/' + item.id);
                const msg = detail?.message || {};
                mails.push({
                    id: item.id,
                    subject: msg.subject || item.subject || '(无主题)',
                    from: msg.from || msg.sender || item.from || '未知',
                    fromName: msg.fromName || '',
                    time: msg.createdAt || msg.receivedAt || item.createdAt || item.receivedAt || '',
                    date: msg.createdAt || msg.receivedAt || item.createdAt || item.receivedAt || '',
                    text: msg.content || '',
                    html: msg.html || '',
                    source: 'moemail'
                });
                ...
                mails.push({
                    ...
                    from: item.from || '未知',
                    fromName: '',
                    time: item.createdAt || item.receivedAt || '',
                    date: item.createdAt || item.receivedAt || '',
                    text: '',
                    html: '',
                    source: 'moemail'
                });
    ...
    function _0x_toggleProviderSections(provider) {
        const moe = document.getElementById('tm-moemail-setting');
        const npc = document.getElementById('tm-npcmail-setting');
        const gpt = document.getElementById('tm-gptmail-setting');
        if (moe) moe.style.display = provider === 'moemail' ? 'block' : 'none';
        if (npc) npc.style.display = provider === 'npcmail' ? 'block' : 'none';
        if (gpt) gpt.style.display = provider === 'gptmail' ? 'block' : 'none';
    }

    function _0x_setRegDomainOptions(domains, preferredDomain = '') {
        const select = document.getElementById('tm-settings-reg-domain');
        if (!select) return;
        select.innerHTML = '<option value="">随机域名</option>';
        ...
        if (preferredDomain) {
            if (!domains.includes(preferredDomain)) {
                ...
                extra.value = preferredDomain;
                extra.innerText = preferredDomain + ' (已保存)';
                select.appendChild(extra);
            }
            select.value = preferredDomain;
        } else {
            select.value = '';
        }
    }

    async function _0x_loadProviderDomains(provider, preferredDomain = '') {
        const select = document.getElementById('tm-settings-reg-domain');
        if (!select) return [];
        select.innerHTML = '<option value="">随机域名</option><option value="" disabled>加载中...</option>';
        let domains = [];
        if (provider === 'moemail') {
            const baseUrl = document.getElementById('tm-settings-moemail-base-url')?.value?.trim() || GM_getValue(_0x_MM_KEYS.API_BASE_URL, _0x_MM_DEFAULT_API_BASE);
            const apiKey = document.getElementById('tm-settings-moemail-key')?.value?.trim() || _0x_mmGetKey();
            if (!apiKey) {
                _0x_setRegDomainOptions([], preferredDomain);
                return [];
            }
            domains = await _0x_mmDomains(baseUrl, apiKey);
        } else {
            domains = await _0x547();
        }
        _0x_setRegDomainOptions(domains, preferredDomain);
        return domains;
    }
    ...
    function _0x_setAddAccountStatus(bg, color, text) {
        const status = document.getElementById('tm-add-status');
        if (!status) return;
    ...
    function _0x_clearAddAccountMeta() {
        const emailInput = document.getElementById('tm-add-email');
        if (!emailInput) return;
        delete emailInput.dataset.emailId;
        delete emailInput.dataset.generatedProvider;
    }

    function _0x_updateAddAccountProviderUI() {
        const provider = document.getElementById('tm-add-provider')?.value || 'manual';
        const generateBtn = document.getElementById('tm-add-generate');
        if (!generateBtn) return;
        const manual = provider === 'manual';
        generateBtn.disabled = manual;
        generateBtn.style.opacity = manual ? '.45' : '1';
        generateBtn.style.cursor = manual ? 'default' : 'pointer';
        generateBtn.innerText = manual ? '手动输入' : '⚡ 生成邮箱';
    }

    function _0x_showAddAccount() {
        const providerSelect = document.getElementById('tm-add-provider');
        const provider = 'manual';
        const emailInput = document.getElementById('tm-add-email');
        const passwordInput = document.getElementById('tm-add-password');
        if (emailInput) {
            emailInput.value = '';
            _0x_clearAddAccountMeta();
        }
        if (passwordInput) passwordInput.value = '';
        if (providerSelect) providerSelect.value = provider;
        _0x_updateAddAccountProviderUI();
        const status = document.getElementById('tm-add-status');
        if (status) {
            status.style.display = 'none';
            status.innerText = '';
        }
        document.getElementById('tm-modal-add-account').classList.add('show');
    }

    async function _0x_generateAddAccountEmail() {
        const provider = document.getElementById('tm-add-provider').value;
        const emailInput = document.getElementById('tm-add-email');
        if (!emailInput) return;
        if (provider === 'manual') {
            _0x_setAddAccountStatus('rgba(251,191,36,.12)', 'rgba(251,191,36,.9)', '⚠️ 当前为手动邮箱模式，请直接输入邮箱地址');
            return;
        }
        try {
            _0x_assertProviderReady(provider);
            const config = {
                prefix: GM_getValue('tm_reg_prefix', ''),
                domain: GM_getValue('tm_reg_domain', '')
            };
            _0x_setAddAccountStatus('rgba(59,130,246,.12)', 'rgba(96,165,250,.9)', '📧 正在生成邮箱...');
            const result = await _0x_createEmailByProvider(provider, config);
            emailInput.value = result.address || '';
            if (result.id) {
                emailInput.dataset.emailId = result.id;
            } else {
                delete emailInput.dataset.emailId;
            }
            emailInput.dataset.generatedProvider = provider;
            _0x_setAddAccountStatus('rgba(16,185,129,.12)', 'rgba(16,185,129,.9)', '✅ 邮箱已生成，可继续保存');
        } catch (e) {
            _0x_setAddAccountStatus('rgba(239,68,68,.12)', 'rgba(239,68,68,.9)', '❌ ' + e.message);
        }
    }

    async function _0x_addAccountManual() {
        const email = document.getElementById('tm-add-email').value.trim();
        const password = document.getElementById('tm-add-password').value.trim();
        const provider = document.getElementById('tm-add-provider').value || 'manual';
        const emailInput = document.getElementById('tm-add-email');

        if (!email || !password) {
            _0x_setAddAccountStatus('rgba(239,68,68,.12)', 'rgba(239,68,68,.9)', '❌ 请输入邮箱和密码');
            return;
        }
        if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
            _0x_setAddAccountStatus('rgba(239,68,68,.12)', 'rgba(239,68,68,.9)', '❌ 邮箱格式不正确');
            return;
        }

        const existing = _0x126().find(a => a.email === email);
        let emailId = (emailInput?.dataset.generatedProvider === provider ? emailInput?.dataset.emailId : null) || existing?.emailId || null;

        try {
            if (provider === 'moemail') {
                _0x_setAddAccountStatus('rgba(59,130,246,.12)', 'rgba(96,165,250,.9)', '🔎 正在匹配 MoeMail 邮箱信息...');
                try {
                    const info = emailId ? { id: emailId } : await _0x_mmFindEmailByAddress(email);
                    if (info?.id) {
                        emailId = info.id;
                    } else {
                        _0x_setAddAccountStatus('rgba(251,191,36,.12)', 'rgba(251,191,36,.9)', '⚠️ 未匹配到 MoeMail emailId，将按普通账号保存');
                        await _0x1ae(800);
                    }
                } catch (e) {
                    _0x_setAddAccountStatus('rgba(251,191,36,.12)', 'rgba(251,191,36,.9)', '⚠️ MoeMail 匹配失败，将继续保存');
                    await _0x1ae(800);
                }
            }

            if (existing) {
                _0x_setAddAccountStatus('rgba(251,191,36,.12)', 'rgba(251,191,36,.9)', '⚠️ 账号已存在，将更新信息');
                await _0x1ae(600);
            } else {
                _0x_setAddAccountStatus('rgba(59,130,246,.12)', 'rgba(96,165,250,.9)', '💾 正在保存账号...');
            }
            ...
            _0x_setAddAccountStatus('rgba(16,185,129,.12)', 'rgba(16,185,129,.9)', '✅ 账号已保存');
            setTimeout(() => {
                document.getElementById('tm-modal-add-account').classList.remove('show');
                _0xff2();
            }, 700);
        } catch (e) {
            _0x_setAddAccountStatus('rgba(239,68,68,.12)', 'rgba(239,68,68,.9)', '❌ ' + e.message);
    ...
    async function _0x_startAutoRegisterFromAccount(email) {
        const account = _0x126().find(a => a.email === email);
        if (!account) {
            alert('未找到账号');
            return;
        }
        if (!account.password) {
            alert('该账号缺少密码，无法启动自动注册');
            return;
        }
        try {
            _0x19d();
            _0xbae('清理缓存...');
            await _0x_resetAuthSiteState();
            await _0x1ae(0x1f4);
            const regData = {
                ...account,
                emailProvider: account.emailProvider || 'manual',
                createdAt: account.createdAt || new Date().toISOString(),
                status: '注册中'
            };
            _0x16a(regData);
            _0x18c(_0x5f.GOTO_SIGNUP);
            _0x137(regData);
            _0x_regSaveGuard();
            _0xbae('正在跳转注册页面...');
            await _0x1ae(0x1f4);
            window.location.href = _0x_AUTH_LOGIN_URL;
        } catch (e) {
            _0xa9d();
            alert('启动自动注册失败: ' + e.message);
            _0x19d();
        }
    }
    ...
    function _0x_fullBackup() {
        const data = {
            _meta: {
                version: '10.2',
                exportedAt: new Date().toISOString(),
                type: 'chatgpt-tools-full-backup'
            },
            accounts: _0x126(),
            settings: {
                emailProvider: GM_getValue('tm_email_provider', 'gptmail'),
                apiKey: GM_getValue(_0x4e.API_KEY, ''),
                gptmailApiKey: GM_getValue('gptmail_api_key', 'gpt-test'),
                moemailApiBaseUrl: GM_getValue(_0x_MM_KEYS.API_BASE_URL, _0x_MM_DEFAULT_API_BASE),
                moemailApiKey: GM_getValue(_0x_MM_KEYS.API_KEY, ''),
                regPrefix: GM_getValue('tm_reg_prefix', ''),
                regDomain: GM_getValue('tm_reg_domain', ''),
                useCd: GM_getValue('tm_use_cd', false),
                customDomains: _0x_getCDs(),
                tmAddr: GM_getValue('tm_tm_addr', ''),
                tmEpin: GM_getValue('tm_tm_epin', ''),
                teamUrl: GM_getValue('tm_team_url', ''),
                teamKey: GM_getValue('tm_team_key', ''),
                autoRedirectTeam: GM_getValue(_0x4e.AUTO_REDIRECT_TEAM, false),
                fabPos: GM_getValue('tm_fab_pos', null)
            }
        };
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'chatgpt-tools-backup-' + new Date().toISOString().slice(0, 10) + '.json';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        _tmToast('已备份全部数据 (' + data.accounts.length + ' 个账号 + 全部设置)', 'success', 2500);
    }

    function _0x_fullRestore() {
        ...
        inp.onchange = function (e) {
            ...
            rd.onload = function (ev) {
                try {
                    ...
                    if (!data._meta || data._meta.type !== 'chatgpt-tools-full-backup') {
                        if (Array.isArray(data) && data.length > 0 && data[0].email) {
                            if (!confirm('检测到这是账号列表文件，包含 ' + data.length + ' 个账号。\n\n是否导入到账号列表？')) return;
                            let added = 0;
                            const cur = _0x126();
                            for (const item of data) {
                                if (item.email && !cur.find(x => x.email === item.email)) {
                                    _0x137(item);
                                    added++;
                                }
                            }
                            _tmToast('已导入 ' + added + ' 个账号', 'success', 2500);
                            return;
                        }
                        alert('无效的备份文件格式。\n请选择由「备份全部数据」导出的 .json 文件');
                        return;
                    }
                    const acCount = data.accounts ? data.accounts.length : 0;
                    const hasSettings = !!data.settings;
                    const info = '备份时间: ' + (data._meta.exportedAt ? new Date(data._meta.exportedAt).toLocaleString() : '未知') + '\n账号数量: ' + acCount + '\n包含设置: ' + (hasSettings ? '是' : '否') + '\n\n⚠️ 恢复将覆盖当前所有数据！确定继续？';
                    if (!confirm(info)) return;
                    if (data.accounts && Array.isArray(data.accounts)) {
                        GM_setValue(_0x4e.ACCOUNTS, data.accounts);
                        try { localStorage.setItem('tm_accounts_backup', JSON.stringify(data.accounts)); } catch (e) { }
                    }
                    if (data.settings) {
                        const s = data.settings;
                        if (s.emailProvider !== undefined) GM_setValue('tm_email_provider', s.emailProvider);
                        if (s.apiKey !== undefined) GM_setValue(_0x4e.API_KEY, s.apiKey);
                        if (s.gptmailApiKey !== undefined) GM_setValue('gptmail_api_key', s.gptmailApiKey);
                        if (s.moemailApiBaseUrl !== undefined) GM_setValue(_0x_MM_KEYS.API_BASE_URL, s.moemailApiBaseUrl || _0x_MM_DEFAULT_API_BASE);
                        if (s.moemailApiKey !== undefined) GM_setValue(_0x_MM_KEYS.API_KEY, s.moemailApiKey);
                        if (s.regPrefix !== undefined) GM_setValue('tm_reg_prefix', s.regPrefix);
                        if (s.regDomain !== undefined) GM_setValue('tm_reg_domain', s.regDomain);
                        if (s.useCd !== undefined) GM_setValue('tm_use_cd', s.useCd);
                        if (s.customDomains !== undefined) _0x_setCDs(s.customDomains);
                        if (s.tmAddr !== undefined) GM_setValue('tm_tm_addr', s.tmAddr);
                        if (s.tmEpin !== undefined) GM_setValue('tm_tm_epin', s.tmEpin);
                        if (s.teamUrl !== undefined) GM_setValue('tm_team_url', s.teamUrl);
                        if (s.teamKey !== undefined) GM_setValue('tm_team_key', s.teamKey);
                        if (s.autoRedirectTeam !== undefined) GM_setValue(_0x4e.AUTO_REDIRECT_TEAM, s.autoRedirectTeam);
                        if (s.fabPos !== undefined && s.fabPos) GM_setValue('tm_fab_pos', s.fabPos);
                    }
                    _tmToast('数据恢复成功！' + acCount + ' 个账号 + 全部设置', 'success', 3000);
                    const sm = document.getElementById('tm-modal-settings');
                    if (sm && sm.classList.contains('show')) {
                        sm.classList.remove('show');
                        setTimeout(() => _0x76a(), 300);
                    }
                } catch (ex) {
                    alert('恢复失败: ' + ex.message);
                }
            };
            rd.readAsText(f);
        };
    ...
    function _0x_isAuthErrorText(text = '') {
        return ['糟糕', '出错了', 'Operation timed out', '操作超时'].some(token => text.includes(token));
    }

    async function _0x_resetAuthSiteState() {
        try {
            localStorage.clear();
            sessionStorage.clear();
        } catch (_ce) { }
        try {
            document.cookie.split(';').forEach(function (_ck) {
                var _n = _ck.split('=')[0].trim();
                if (!_n) return;
                document.cookie = _n + '=;expires=Thu,01 Jan 1970 00:00:00 GMT;path=/';
                document.cookie = _n + '=;expires=Thu,01 Jan 1970 00:00:00 GMT;path=/;domain=.chatgpt.com';
                document.cookie = _n + '=;expires=Thu,01 Jan 1970 00:00:00 GMT;path=/;domain=.openai.com';
                document.cookie = _n + '=;expires=Thu,01 Jan 1970 00:00:00 GMT;path=/;domain=.auth.openai.com';
            });
        } catch (_ce) { }
    }

    async function _0x_retryAuthFlowOrAbort(data, retryCount) {
        if (retryCount <= 5) {
            _0xbae('检测到错误页面，重试 (' + retryCount + '/5)...');
            _0x16a({ ...data, _retries: retryCount });
            await _0x1ae(0x5dc);
            window.location.href = _0x_AUTH_LOGIN_URL;
            return true;
        }
    ...
    function _0x_getGlobalProvider() {
        return GM_getValue('tm_email_provider', 'gptmail');
    }

    function _0x_getAccountProvider(acc) {
        return acc?.emailProvider === 'tempmail'
            ? _0x_getGlobalProvider()
            : (acc?.emailProvider || _0x_getGlobalProvider());
    }

    function _0x_extractNumericCode(text = '') {
    ...
    async function _0x_fetchGptmailCode(email) {
        const gptK = GM_getValue('gptmail_api_key', 'gpt-test');
        const response = await _gptReq('/emails?email=' + encodeURIComponent(email), gptK);
        const mails = Array.isArray(response) ? response : (response?.emails || response?.data?.emails || []);
        for (const mail of mails) {
            if (mail?.id) {
                const detail = await _gptReq('/email/' + mail.id, gptK);
                const code = _0x_extractNumericCode((detail.content || '') + ' ' + (detail.html_content || '') + ' ' + (detail.subject || ''));
                if (code) return code;
                continue;
            }
            const code = _0x_extractNumericCode((mail.content || '') + ' ' + (mail.html_content || '') + ' ' + (mail.subject || ''));
            if (code) return code;
        }
        return null;
    }

    async function _0x_fetchGptmailMails(email) {
        const gptK = GM_getValue('gptmail_api_key', 'gpt-test');
        const response = await _gptReq('/emails?email=' + encodeURIComponent(email), gptK);
        const mails = Array.isArray(response) ? response : (response?.emails || response?.data?.emails || []);
        return mails.map(mail => ({
            ...
            from: mail.from_address || mail.from || '未知',
            fromName: '',
            time: mail.created_at || mail.received_at || '',
            date: mail.created_at || mail.received_at || '',
            text: mail.content || '',
            html: mail.html_content || '',
            isRead: mail.is_read || false,
            source: 'gptmail'
        }));
    }

    async function _0x_fetchNpcmailMails(email) {
        const response = await _0x_npcMails(email);
        const mails = Array.isArray(response) ? response : (response?.messages || []);
        return mails.map((mail, index) => ({
            id: mail.id || index,
            subject: mail.subject || '(无主题)',
            from: mail.sender || mail.from || '未知',
            fromName: '',
            time: mail.received_at ? new Date(mail.received_at).toLocaleString() : '',
    ...
    async function _0x_fetchProviderMails(provider, email, options = {}) {
        if (provider === 'moemail') {
            return _0x_mmFetchMails(email, options.emailId);
        }
        if (provider === 'manual') {
            throw new Error('手动邮箱不支持自动收件');
        }
        if (provider === 'npcmail') {
            return _0x_fetchNpcmailMails(email);
        }
        return _0x_fetchGptmailMails(email);
    }

    async function _0x_fetchProviderCode(provider, email, options = {}) {
        if (provider === 'tempmail') {
            return options.tmAddr ? _0x_tmCode(options.tmAddr, options.tmEpin || '') : null;
        }
        if (provider === 'manual') {
            throw new Error('手动邮箱不支持自动取码');
        }
        if (provider === 'moemail') {
            return _0x_mmExtractCode(email, options.emailId);
        }
        if (provider === 'npcmail') {
            return _0x115(email);
        }
        return _0x_fetchGptmailCode(email);
    }

    function _0x_assertProviderReady(provider) {
        if (provider === 'manual') return;
        if (provider === 'moemail') {
            const baseUrl = GM_getValue(_0x_MM_KEYS.API_BASE_URL, _0x_MM_DEFAULT_API_BASE);
            const key = GM_getValue(_0x_MM_KEYS.API_KEY, '');
            if (!key) throw new Error('请先配置 MoeMail API Key');
            _0x_mmNormBaseUrl(baseUrl);
            return;
        }
        if (provider === 'npcmail') {
            const key = GM_getValue(_0x4e.API_KEY, '');
            if (!key) throw new Error('请先配置 NPCmail 密钥或启用自定义域名模式');
            return;
        }
        const key = GM_getValue('gptmail_api_key', 'gpt-test');
        if (!key) throw new Error('请先配置 GPTMail 密钥');
    }

    async function _0x_fetchFullAuthSession(maxAttempts = 12, delayMs = 1000) {
        for (let i = 0; i < maxAttempts; i++) {
            try {
                const response = await fetch(_0x_AUTH_SESSION_URL, { credentials: 'include' });
            ...
            if (i < maxAttempts - 1) {
                await _0x1ae(delayMs);
            }
        }
        return null;
    }

    async function _0x_finalizeRegisteredAccount(account) {
        let completed = { ...account, status: '注册完成' };
        try {
            const sessionData = await _0x_fetchFullAuthSession();
            if (sessionData) {
                completed = {
                    ...completed,
                    authSession: sessionData,
                    authSessionFetchedAt: new Date().toISOString(),
                    accessToken: sessionData.accessToken || completed.accessToken,
                    sessionEmail: sessionData?.user?.email || sessionData?.email || completed.sessionEmail,
                    authAccountId: sessionData?.account?.id || completed.authAccountId,
                    authUserId: sessionData?.user?.id || completed.authUserId
                };
            }
        } catch (e) { }
        _0xa9d();
    ...
    async function _0x_createEmailByProvider(provider, config = {}) {
        if (provider === 'manual') {
            throw new Error('手动邮箱模式不支持自动生成邮箱');
        }
        if (provider === 'moemail') {
            return _0x_mmCreateEmail(config.domain || null, config.prefix || null);
        }
        if (provider === 'npcmail') {
            return _0xf3(config.domain || null, config.prefix || null);
        }
        const key = GM_getValue('gptmail_api_key', 'gpt-test');
        const payload = {};
        ...
        if (Object.keys(payload).length > 0) {
            const res = await _gptReqPost('/generate-email', payload, key);
            return { address: res.email };
        }
        const res = await _gptReq('/generate-email', key);
        return { address: res.email };
    }
...
<div id="tm-moemail-setting" style="display:none">
<label class="tm-lbl">MoeMail API Base URL</label><input type="text" class="tm-inp" id="tm-settings-moemail-base-url" placeholder="例如 https://sall.cc" style="margin-bottom:4px"><div style="font-size:10px;color:rgba(255,255,255,.45);margin-top:0;margin-bottom:8px">必须填写完整协议头，例如 https://example.com</div>
<label class="tm-lbl">MoeMail API Key</label><input type="text" class="tm-inp" id="tm-settings-moemail-key" placeholder="输入 MoeMail API 密钥">
</div>
...
<div class="tm-md-f"><button class="tm-btn tm-btn-s" data-close>关闭</button><button class="tm-btn tm-btn-s" id="tm-settings-test" style="background:rgba(59,130,246,.15);color:rgba(96,165,250,.9);border-color:rgba(59,130,246,.2)">测试邮箱连接</button><button class="tm-btn tm-btn-s" id="tm-settings-test-team" style="background:rgba(245,158,11,.12);color:rgba(251,191,36,.9);border-color:rgba(245,158,11,.2)">测试 Team</button><button class="tm-btn tm-btn-p" id="tm-settings-save">💾 保存</button></div>
            ...
            m.querySelectorAll('[data-close]').forEach(b => b.onclick = () => m.classList.remove('show'));
            document.getElementById('tm-settings-test').onclick = async function () {
                const st = document.getElementById('tm-settings-status');
                const prv = document.getElementById('tm-settings-email-provider').value;
                const currentDomain = document.getElementById('tm-settings-reg-domain').value;
                this.innerText = '测试中...';
                try {
                    if (prv === 'moemail') {
                        const baseUrl = document.getElementById('tm-settings-moemail-base-url').value.trim();
                        const apiKey = document.getElementById('tm-settings-moemail-key').value.trim();
                        if (!apiKey) throw new Error('请输入 MoeMail API Key');
                        const domains = await _0x_mmDomains(baseUrl, apiKey);
                        _0x_setRegDomainOptions(domains, currentDomain);
                        st.style.display = 'block'; st.style.background = 'rgba(16,185,129,.12)'; st.style.color = 'rgba(16,185,129,.9)';
                        st.innerText = '✅ MoeMail 连接成功！可用域名: ' + domains.join(', ');
                    } else if (prv === 'npcmail') {
                        const k = document.getElementById('tm-settings-apikey').value.trim();
                        if (!k) throw new Error('请输入 NPCmail 密钥');
                        GM_setValue(_0x4e.API_KEY, k);
                        const i = await _0xe2();
                        await _0x_loadProviderDomains('npcmail', currentDomain);
                        st.style.display = 'block'; st.style.background = 'rgba(16,185,129,.12)'; st.style.color = 'rgba(16,185,129,.9)';
                        st.innerText = '✅ NPCmail 连接成功！剩余调用: ' + i.remaining_calls;
                    } else {
                        const k = document.getElementById('tm-settings-gptmail-key').value.trim();
                        if (!k) throw new Error('请输入 GPTMail 密钥');
                        GM_setValue('gptmail_api_key', k);
                        const i = await _gptReq('/stats', k);
                        await _0x_loadProviderDomains('gptmail', currentDomain);
                        st.style.display = 'block'; st.style.background = 'rgba(16,185,129,.12)'; st.style.color = 'rgba(16,185,129,.9)';
                        st.innerText = '✅ GPTMail 连接成功！系统收件数: ' + (i.total_emails || 0);
                    }
                } catch (e) {
                    st.style.display = 'block'; st.style.background = 'rgba(239,68,68,.12)'; st.style.color = 'rgba(239,68,68,.9)';
                    st.innerText = '❌ ' + e.message;
                }
                this.innerText = '测试邮箱连接';
            };
            document.getElementById('tm-settings-email-provider').onchange = async function () {
                _0x_toggleProviderSections(this.value);
                try {
                    await _0x_loadProviderDomains(this.value, '');
                } catch (e) {
                    const st = document.getElementById('tm-settings-status');
                    st.style.display = 'block'; st.style.background = 'rgba(251,191,36,.12)'; st.style.color = 'rgba(251,191,36,.9)';
                    st.innerText = '⚠️ 域名加载失败: ' + e.message;
                }
            };
            document.getElementById('tm-settings-moemail-base-url').onchange = async function () {
                if (document.getElementById('tm-settings-email-provider').value !== 'moemail') return;
                try { await _0x_loadProviderDomains('moemail', document.getElementById('tm-settings-reg-domain').value); } catch (_) { }
            };
            document.getElementById('tm-settings-moemail-key').onchange = async function () {
                if (document.getElementById('tm-settings-email-provider').value !== 'moemail') return;
                try { await _0x_loadProviderDomains('moemail', document.getElementById('tm-settings-reg-domain').value); } catch (_) { }
            };
            ...
            document.getElementById('tm-settings-save').onclick = function () {
                const prEl = document.getElementById('tm-settings-email-provider');
                if (prEl) {
                    GM_setValue('tm_email_provider', prEl.value);
                    console.log('[Settings] 邮箱提供商已保存:', prEl.value);
                }
                GM_setValue(_0x_MM_KEYS.API_BASE_URL, document.getElementById('tm-settings-moemail-base-url').value.trim() || _0x_MM_DEFAULT_API_BASE);
                GM_setValue(_0x_MM_KEYS.API_KEY, document.getElementById('tm-settings-moemail-key').value.trim());
                const gk = document.getElementById('tm-settings-gptmail-key');
                if (gk) GM_setValue('gptmail_api_key', gk.value.trim());
                GM_setValue(_0x4e.API_KEY, document.getElementById('tm-settings-apikey').value.trim());
                GM_setValue(_0x4e.AUTO_REDIRECT_TEAM, document.getElementById('tm-settings-auto-redirect').checked);
                GM_setValue('tm_reg_prefix', document.getElementById('tm-settings-reg-prefix').value.trim());
                GM_setValue('tm_reg_domain', document.getElementById('tm-settings-reg-domain').value);
                GM_setValue('tm_use_cd', document.getElementById('tm-settings-use-cd').checked);
                GM_setValue('tm_tm_addr', document.getElementById('tm-settings-tm-addr').value.trim());
                GM_setValue('tm_tm_epin', document.getElementById('tm-settings-tm-epin').value.trim());
                GM_setValue('tm_team_url', document.getElementById('tm-settings-team-url').value.trim());
                GM_setValue('tm_team_key', document.getElementById('tm-settings-team-key').value.trim());
                const _oldSyncKey = GM_getValue('tm_sync_apikey', '');
                const _newSyncUrl = document.getElementById('tm-settings-sync-url').value.trim();
                const _newSyncKey = document.getElementById('tm-settings-sync-apikey').value.trim();
                GM_setValue('tm_sync_url', _newSyncUrl);
                GM_setValue('tm_sync_apikey', _newSyncKey);
                if (_newSyncKey && _newSyncUrl && _newSyncKey !== _oldSyncKey) {
                    setTimeout(async () => {
                        try {
                            const r = await _syncPushAll();
                            _tmToast('API密钥已设置，自动上传了 ' + r.count + ' 个账号', 'success', 3000);
                            console.log('[CloudSync] 首次设置密钥，自动全量上传完成');
                        } catch (e) {
                            console.warn('[CloudSync] 首次全量上传失败:', e.message);
                        }
                    }, 500);
                }
...
<div class="tm-md-f" style="flex-wrap:wrap;gap:4px"><button class="tm-btn tm-btn-s" data-close>关闭</button><button class="tm-btn tm-btn-sm" id="tm-acc-add" style="background:rgba(245,158,11,.14);color:rgba(251,191,36,.95);border-color:rgba(245,158,11,.18)">➕ 添加</button><button class="tm-btn tm-btn-sm" id="tm-acc-export" style="background:rgba(59,130,246,.15);color:rgba(96,165,250,.9);border-color:rgba(59,130,246,.2)">📤 导出</button><button class="tm-btn tm-btn-sm" id="tm-acc-import" style="background:rgba(16,185,129,.15);color:rgba(16,185,129,.9);border-color:rgba(16,185,129,.2)">📥 导入</button><button class="tm-btn tm-btn-sm" id="tm-acc-import-email" style="background:rgba(168,85,247,.15);color:rgba(192,132,252,.9);border-color:rgba(168,85,247,.2)">📬 邮箱导入</button><button class="tm-btn tm-btn-sm" id="tm-acc-export-email" style="background:rgba(6,182,212,.12);color:rgba(34,211,238,.9);border-color:rgba(6,182,212,.15)">📧 邮箱导出</button></div>
        ...
        if (!document.getElementById('tm-modal-add-account')) {
            const m = document.createElement('div'); m.id = 'tm-modal-add-account'; m.className = 'tm-ov'; m.innerHTML = `
<div class="tm-md" style="max-width:400px">
<div class="tm-md-h"><div class="tm-md-t">➕ 手动添加账号</div><button class="tm-md-x" data-close>✕</button></div>
<div class="tm-md-b">
<div class="tm-fld"><label class="tm-lbl">邮箱</label><div style="display:flex;gap:6px"><input type="email" class="tm-inp" id="tm-add-email" placeholder="输入邮箱地址或点击右侧生成" style="flex:1"><button class="tm-btn tm-btn-s tm-btn-sm" id="tm-add-generate" style="white-space:nowrap">⚡ 生成邮箱</button></div></div>
<div class="tm-fld"><label class="tm-lbl">密码</label><input type="text" class="tm-inp" id="tm-add-password" placeholder="输入密码"></div>
<div class="tm-fld"><label class="tm-lbl">邮箱来源</label><select class="tm-inp tm-sel" id="tm-add-provider"><option value="manual">不指定（手动邮箱）</option><option value="moemail">MoeMail</option><option value="npcmail">NPCmail</option><option value="gptmail">GPTMail</option></select><div style="font-size:10px;color:rgba(255,255,255,.45);margin-top:4px">手动邮箱可直接输入任意邮箱；选择其他来源后可点“生成邮箱”自动创建</div></div>
<div id="tm-add-status" style="display:none;padding:8px 10px;border-radius:8px;font-size:12px;margin-top:8px"></div>
</div>
<div class="tm-md-f"><button class="tm-btn tm-btn-s" data-close>关闭</button><button class="tm-btn tm-btn-p" id="tm-add-save">💾 保存</button></div>
</div>`;
            document.body.appendChild(m);
            m.querySelectorAll('[data-close]').forEach(b => b.onclick = () => m.classList.remove('show'));
            document.getElementById('tm-add-generate').onclick = _0x_generateAddAccountEmail;
            document.getElementById('tm-add-save').onclick = _0x_addAccountManual;
            document.getElementById('tm-add-email').addEventListener('input', _0x_clearAddAccountMeta);
        }
...
<button class="tm-btn tm-btn-p" id="tm-success-copy">📋 复制</button>
    ...
    async function _0x_fetchAllMails(email) {
        const ac = _0x126();
        const acc = ac.find(a => a.email === email);
        const isCD = acc && acc.customDomain;
        const isTmDirect = email.startsWith('__tm__');
        const tmAddr = GM_getValue('tm_tm_addr', '');
        const tmEpin = GM_getValue('tm_tm_epin', '');

        if (isTmDirect || isCD) {
            const useAddr = isTmDirect ? email.replace('__tm__', '') : tmAddr;
            if (!useAddr) throw new Error('请先在设置中配置 TempMail 取件邮箱');
            const inbox = await _0x_tmInbox(useAddr, tmEpin);
            if (!inbox.mail_list || inbox.mail_list.length === 0) return [];
            const mails = [];
            for (const m of inbox.mail_list) {
                try {
                    const det = await _0x_tmMail(m.mail_id, useAddr, tmEpin);
                    mails.push({ id: m.mail_id, subject: det.subject || m.subject || '(无主题)', from: det.from_mail || m.from_mail || '未知', fromName: det.from_name || m.from_name || '', time: m.time || '', date: det.date || '', text: det.text || '', html: det.html || '', source: 'tempmail' });
                } catch (e) {
                    mails.push({ id: m.mail_id, subject: m.subject || '(无主题)', from: m.from_mail || '未知', fromName: m.from_name || '', time: m.time || '', text: '', html: '', source: 'tempmail' });
                }
            }
            _inboxCache = mails;
            return mails;
        ...
        const prv = _0x_getAccountProvider(acc);
        try {
            const mails = await _0x_fetchProviderMails(prv, email, { emailId: acc?.emailId });
            _inboxCache = mails;
            return mails;
        } catch (e) {
            const providerLabel = prv === 'moemail' ? 'MoeMail' : (prv === 'npcmail' ? 'NPCmail' : 'GPTMail');
            throw new Error(providerLabel + ' 获取失败: ' + e.message);
        }
    }
...
${m.html ? '<button class="tm-btn tm-btn-s tm-btn-sm" id="tm-mail-toggle-raw">🔄 切换原始/解析</button>' : ''}
    ...
    async function _0x76a() {
        let defPrv = GM_getValue('tm_email_provider', 'gptmail');
        console.log('[Settings] GM_getValue tm_email_provider =', defPrv);
        if (!['moemail', 'npcmail', 'gptmail'].includes(defPrv)) defPrv = 'gptmail';

        const prEl = document.getElementById('tm-settings-email-provider');
        if (prEl) {
            for (let i = 0; i < prEl.options.length; i++) {
                prEl.options[i].selected = (prEl.options[i].value === defPrv);
            }
            prEl.value = defPrv;
            console.log('[Settings] 下拉框设置为:', prEl.value);
        }

        _0x_toggleProviderSections(defPrv);

        const mmBase = document.getElementById('tm-settings-moemail-base-url');
        const mmKey = document.getElementById('tm-settings-moemail-key');
        if (mmBase) mmBase.value = GM_getValue(_0x_MM_KEYS.API_BASE_URL, _0x_MM_DEFAULT_API_BASE);
        if (mmKey) mmKey.value = GM_getValue(_0x_MM_KEYS.API_KEY, '');

        document.getElementById('tm-settings-apikey').value = GM_getValue(_0x4e.API_KEY, '');
        const gkp = document.getElementById('tm-settings-gptmail-key');
        if (gkp) gkp.value = GM_getValue('gptmail_api_key', 'gpt-test');

        document.getElementById('tm-settings-auto-redirect').checked = GM_getValue(_0x4e.AUTO_REDIRECT_TEAM, !1);
        document.getElementById('tm-settings-reg-prefix').value = GM_getValue('tm_reg_prefix', '');
        document.getElementById('tm-settings-team-url').value = GM_getValue('tm_team_url', '');
        document.getElementById('tm-settings-team-key').value = GM_getValue('tm_team_key', '');

        const ucd = GM_getValue('tm_use_cd', false);
        document.getElementById('tm-settings-use-cd').checked = ucd;
        document.getElementById('tm-cd-section').style.display = ucd ? 'block' : 'none';
        document.getElementById('tm-settings-tm-addr').value = GM_getValue('tm_tm_addr', '');
        document.getElementById('tm-settings-tm-epin').value = GM_getValue('tm_tm_epin', '');
        _0x_renderCDs();

        document.getElementById('tm-settings-sync-url').value = GM_getValue('tm_sync_url', '');
        document.getElementById('tm-settings-sync-apikey').value = GM_getValue('tm_sync_apikey', '');

        const sd = GM_getValue('tm_reg_domain', '');
        try {
            await _0x_loadProviderDomains(defPrv, sd || '');
        } catch (e) {
            console.warn('[Settings] 加载域名失败:', e.message);
            _0x_setRegDomainOptions([], sd || '');
        }

        document.getElementById('tm-settings-status').style.display = 'none';
        document.getElementById('tm-modal-settings').classList.add('show');
    }
    ...
    function _0xff2(page) {
        ...
        h += slice.map((a, idx) => {
            const i = start + idx;
            const statusText = a.status || '已创建';
            const createdText = a.createdAt ? new Date(a.createdAt).toLocaleString() : '未知时间';
            const canAutoRegister = statusText !== '注册完成';
            return `
<div class="tm-card">
<div class="tm-card-t">${a.email}</div>
<div class="tm-card-sub">密码: ${a.password}</div>
<div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin-top:6px">
<span style="font-size:10px;color:rgba(255,255,255,.42)">状态</span>
<span style="font-size:10px;color:rgba(255,255,255,.92);background:rgba(59,130,246,.16);border:1px solid rgba(96,165,250,.2);border-radius:999px;padding:2px 8px;line-height:1.4">${statusText}</span>
</div>
<div class="tm-card-meta">创建时间: ${createdText}</div>
...
${canAutoRegister ? '<button class="tm-btn tm-btn-sm" style="background:rgba(16,185,129,.15);color:rgba(16,185,129,.92);border-color:rgba(16,185,129,.2)" data-action="autoreg" data-email="' + a.email + '">🚀 注册</button>' : ''}
...
<button class="tm-btn tm-btn-s tm-btn-sm" id="tm-pg-last" ${page >= pages ? 'disabled style="opacity:.4;cursor:default"' : ''}>末页</button>
    ...
    document.addEventListener('click', async function (e) {
        ...
        if (a === 'code') {
            ...
            try {
                b.innerText = '...';
                let c = null;
                const acc = _0x126().find(item => item.email === em);
                const isTM = em.includes('__tm__') || !!acc?.customDomain;
                const curProv = _0x_getAccountProvider(acc);
                if (isTM) {
                    const tmAddr = em.includes('__tm__') ? em.replace('__tm__', '') : (acc?.tmAddr || GM_getValue('tm_tm_addr', ''));
                    const tmEpin = acc?.tmEpin || GM_getValue('tm_tm_epin', '');
                    if (tmAddr) c = await _0x_fetchProviderCode('tempmail', em, { tmAddr, tmEpin });
                } else {
                    c = await _0x_fetchProviderCode(curProv, em, { emailId: acc?.emailId });
                }
                if (c) {
                    _0x7b(c);
                    alert('验证码: ' + c + '\n已复制');
                } else {
                    const providerLabel = curProv === 'manual' ? '手动邮箱' : curProv;
                    alert(providerLabel === '手动邮箱' ? '手动邮箱请在注册流程中手动输入验证码' : '未找到验证码');
                }
            } catch (ex) {
                alert('错误: ' + ex.message);
            }
        ...
        if (a === 'autoreg') {
            const em = b.dataset.email;
            if (!confirm('使用该账号启动自动注册？')) return;
            await _0x_startAutoRegisterFromAccount(em);
            return;
        }
    ...
    async function _0x659() {
        const ucd = GM_getValue('tm_use_cd', false);
        const cds = _0x_getCDs();
        if (ucd && cds.length > 0) {
            const tmA = GM_getValue('tm_tm_addr', '');
            if (!tmA) { alert('请先在设置中配置 TempMail 取件邮箱'); _0x76a(); return; }
            try {
                _0x19d();
                _0xbae('清理缓存...');
                await _0x_resetAuthSiteState();
                await _0x1ae(0x1f4);
                _0xbae('正在准备自定义域名邮箱...');
                const dom = cds[Math.floor(Math.random() * cds.length)];
                const pfx = GM_getValue('tm_reg_prefix', '') || _0x_randPfx(12);
                const email = pfx + '@' + dom;
                const rd = { email: email, password: _0xae(), ..._0xbf(), birthday: _0xc0(), createdAt: new Date().toISOString(), status: '注册中', emailProvider: 'tempmail', customDomain: true, tmAddr: tmA, tmEpin: GM_getValue('tm_tm_epin', '') };
                _0x16a(rd);
                _0x18c(_0x5f.GOTO_SIGNUP);
                _0x137(rd);
                _0x_regSaveGuard();
        ...
        const prv = _0x_getGlobalProvider();
        try {
            _0x_assertProviderReady(prv);
        } catch (e) {
            alert(e.message);
            _0x76a();
            return;
        }
        const p = GM_getValue('tm_reg_prefix', '');
        const d = GM_getValue('tm_reg_domain', '');
        await _0x87b({ prefix: p, domain: d });
    }

    async function _0x87b(c = {}) {
        try {
            _0x19d();
            _0xbae('清理缓存...');
            await _0x_resetAuthSiteState();
            ...
            _0xbae('正在创建邮箱...');
            const prv = _0x_getGlobalProvider();
            const ed = await _0x_createEmailByProvider(prv, c);
            const rd = { email: ed.address, emailId: ed.id || null, password: _0xae(), ..._0xbf(), birthday: _0xc0(), createdAt: new Date().toISOString(), status: '注册中', emailProvider: prv };
            _0x16a(rd);
            _0x18c(_0x5f.GOTO_SIGNUP);
            _0x137(rd);
            _0x_regSaveGuard();
            ...
            _0xa9d();
            alert('创建邮箱失败: ' + e.message);
            _0x19d();
        }
    }
    async function _0x98d(d) { function _vp(s) { const els = document.querySelectorAll(s); for (const el of els) { const rc = el.getBoundingClientRect(); if (rc.width > 0 && rc.height > 0) { const cs = window.getComputedStyle(el); if (cs.visibility !== 'hidden' && cs.display !== 'none') return el; } } return null; } _0xbae('等待密码框加载...'); let pi = null; let _wt = 0; while (!pi && _wt < 60) { _wt++; var _etp = document.body ? document.body.textContent : ''; if (_0x_isAuthErrorText(_etp)) { const shouldRetry = await _0x_retryAuthFlowOrAbort(d, (d._retries || 0) + 1); if (shouldRetry) return; return; } _0xbae('等待密码框... (' + _wt + '/60)'); await _0x1ae(0x3e8); pi = _vp('input[type="password"], input[name="password"]'); } if (!pi) { _0xbae('密码框未找到'); return; } _0xbae('正在填写密码...'); await _0x1ae(0x1f4); _0x1c0(pi, d.password); _0x137({ ...d, status: '密码已填写' }); await _0x1ae(0x3e8); let cb = document.querySelector('button[type="submit"]') || _0x1f3('button', ['Continue', '继续', 'Next']); if (cb) { _0x1e2(cb); _0x18c(_0x5f.WAIT_CODE); _0xbae('等待验证码页面...'); let _tw = 0; let _codeReady = false; while (_tw < 60) { _tw++; await _0x1ae(0x3e8); var _etw = document.body ? document.body.textContent : ''; if (_0x_isAuthErrorText(_etw)) { const shouldRetry = await _0x_retryAuthFlowOrAbort(d, (d._retries || 0) + 1); if (shouldRetry) return; return; } const _ci = document.querySelectorAll('input[type="text"][maxlength="1"]'); const _cs = document.querySelector('input[autocomplete="one-time-code"], input[name="code"]'); if (_ci.length >= 4 || _cs) { _0xbae('验证码页面已加载'); _codeReady = true; break; } _0xbae('等待页面切换... (' + _tw + '/60)'); } if (!_codeReady) { _0xbae('验证码页面未加载'); return; } await _0x1ae(0x7d0); await _0xa9e(d); } }
    async function _0xa9e(d) {
        _0xbae('等待验证码邮件...');
        let c = null;
        const isTM = d.customDomain && d.tmAddr;
        const curProv = _0x_getAccountProvider(d);
        for (let i = 0x0; i < 0x3c; i++) {
            var _eta = document.body ? document.body.textContent : '';
            if (_0x_isAuthErrorText(_eta)) {
                const shouldRetry = await _0x_retryAuthFlowOrAbort(d, (d._retries || 0) + 1);
                if (shouldRetry) return;
                return;
            }
            _0xbae('等待验证码... (' + (i + 0x1) + '/60)' + (isTM ? ' [TempMail:' + d.tmAddr + ']' : (' [Email:' + d.email + ']')));
            if (isTM) {
                c = await _0x_fetchProviderCode('tempmail', d.email, { tmAddr: d.tmAddr, tmEpin: d.tmEpin || '' });
            } else if (curProv === 'manual') {
                const input = prompt('请从邮箱 ' + d.email + ' 查看验证码并输入');
                c = _0x_extractNumericCode(String(input || '').trim());
                if (!c) {
                    throw new Error('未输入有效验证码');
                }
            } else {
                try {
                    c = await _0x_fetchProviderCode(curProv, d.email, { emailId: d.emailId });
                } catch (x) { }
            }
            if (c) {
                _0xbae('验证码获取成功: ' + c);
                ...
                _0xbae('第' + (i + 1) + '次未获取到验证码');
            }
            await _0x1ae(0xbb8);
        }
        if (c) {
            _0x137({ ...d, status: '验证码已获取', verificationCode: c });
            _0x16a({ ...d, verificationCode: c });
            _0x18c(_0x5f.FILL_CODE);
            await _0xbaf(c, d);
        } else {
            _0x137({ ...d, status: '验证码超时' });
